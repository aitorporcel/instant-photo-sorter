...
def main(num_epochs: int, batch_size: int, train_split: float):
    ...
    evaluate_model_per_class(
        model_trained, val_loader, criterion, device, test_eval=False
    )
    evaluate_model_per_class(
        model_trained, test_loader, criterion, device, test_eval=True
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.7,
        help="Percentage of data to use for training (rest is for validation).",
    )
